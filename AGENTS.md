...
- **NEXT:** live-verify the Greenbone pipeline against the running install (`.venv/bin/python scripts/update_greenbone.py` with `GREENBONE_HOST/PORT/USER/PASSWORD`), then optionally schedule cache refresh (5b §4.3 pattern) and add service→product alias mapping for banner-name lookups.
...
- **Greenbone CVE cache envs (scanner):** `GREENBONE_FEED_PATH` (cache file; default `scanner/data/greenbone_cves.json` — dir gitignored) + live-fetch-only `GREENBONE_HOST`/`GREENBONE_PORT`/`GREENBONE_USER`/`GREENBONE_PASSWORD` for `scripts/update_greenbone.py` (python-gvm, lazy-imported).
