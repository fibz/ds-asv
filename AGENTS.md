...
- **Runnability status (verified 2026-09-01/02, fresh checks):**
  - ✅ Portal backend + API (Phases 1–4) — done, 335/335 portal tests green
  ...
  - ❌ Prod staff attestation — `resolveTenantContext` hardcodes `isStaff:false` (portal/src/lib/tenant.ts:81); no report can be FINAL in prod until staff identity lands (fail-closed by design)
...
- **NEXT:** Phase 6 = scanner real-scan tooling on this host (install `python-nmap` into `scanner/.venv`, install `testssl.sh` into `scanner/scripts/`, smoke-test a real black-box scan vs `127.0.0.1`, re-run scanner pytest). Then: prod staff attestation wiring, then the CVE source.
