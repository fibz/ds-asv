...
The contract Phase 3 tests is: (1) `POST /api/v1/scans/{scanId}/findings` accepting `Authorization: Bearer <manifest>` with `{ findings: [...] }` (shape in the OpenAPI `FindingIngest` schema) — the real scanner verifies the manifest, runs the black-box scan per target, and posts findings; (2) `PATCH /api/v1/scans/{scanId}` `{status: RUNNING|COMPLETED|FAILED}` for lifecycle updates; (3) the manifest format (payload `<b64url JSON>. <HMAC hex>`) issued by `issueScanManifest`. Phase 3b implements the scanner-side consumer in `scanner/` (FastAPI) against these three contracts, replacing `simulatedScanner`, and wires the dispatch trigger. The CVE/scoring in `scanner/app/scoring/` feeds `cveId`/severity into the ingested findings.

> **User directive (2026-08-31, supersedes the self-hosted NVD mirror):** Phase 3b's CVE/scoring source is **NOT** the self-hosted NVD mirror originally assumed in the design spec §7. The user is evaluating **VulDB** as the primary CVE/threat feed, and notes Kali (nmap vuln scripts) and Greenbone/OpenVAS each ship their own **local** vulnerability DBs — so no separate self-hosted CVE DB is required for their output. **Rationale:** the CVE data feeds the user's **SOC + threat detection**, so it must be current and maintained (VulDB) or come from the scanner's own local DBs, rather than a stale home-grown NVD mirror. **Phase 3b must decide/confirm the scoring source before building:** prefer VulDB (or the scanner's local Kali/Greenbone DBs) over implementing `nvd_loader.py` against a raw NVD feed; wire the chosen source into `scanner/app/scoring/` (the current `cpe_mapper.py` hardcoded table + `pci_rules.py` canned CVSS numbers are a stopgap, not the real source). This does not change the manifest/ingestion contract — `cveId`/`severity`/`pciSeverity` are still just values the scanner posts to the portal.
