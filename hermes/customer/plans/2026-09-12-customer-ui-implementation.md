...
10. **Raising a dispute is not implemented.** `ReportDetail` shows findings read-only; there is no dispute form or mutation. The spec (§5.6) promises a dispute entry point, so this is a **follow-up task** requiring the `apiPost` layer plus a form, confirmation, and cache invalidation. Do not render a dispute control that does nothing.
