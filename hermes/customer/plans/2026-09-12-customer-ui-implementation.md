...
8. **A visible explanation, not `sr-only`, for blocked actions.** `Button`'s `disabledReason` renders into a screen-reader-only node, which `getByText` does not reliably match — and a sighted user cannot see it either. Where the design promises the reason in the UI (Scans' "New scan"), render it as visible text as well as passing `disabledReason`.
