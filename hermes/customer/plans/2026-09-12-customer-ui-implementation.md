...
7. **Our report-finality rule was stricter than the server's** (found while building Home, fixed before Reports was built on it). The portal decides finality *per report*: the version that report records must itself be approved (`portal/src/app/customer/reports/page.tsx`). The first implementation instead compared each report against the **newest** approved version, so a report backed by an older-but-approved `v3` read "not final" the moment `v4` was approved — contradicting the server and the PDF the customer holds. The gate input is now resolved per report by `approvedScopeVersionIdFor(reportScopeVersionId, versions)`. **Do not feed a single "current approved version id" into the gate.**
8. **A blocked action needs a visible reason, tied to the control.** `Button`'s `disabledReason` renders into an `sr-only` node: invisible to sighted users, and rendering it *alongside* a visible copy of the same sentence puts the text in the DOM twice — which duplicates it for screen readers and makes the phrase ambiguous to `getByText`. The correct pattern, now used by Scans: **one visible explanation** carrying an `id`, with the button pointing at it via `aria-describedby`. `Button` was fixed to pass a caller-supplied `aria-describedby` through instead of clobbering it (`rest` spread happened after it, so it was being overwritten with `undefined`).
9. **`useApprovedScopeVersionId()` lost its consumers** when the gate moved to per-report resolution. If Task 12's Scope screen does not use it, delete the hook and its test mocks rather than leaving dead code.
10. **Raising a dispute is not implemented.** `ReportDetail` shows findings read-only; there is no dispute form or mutation. The spec (§5.6) promises a dispute entry point, so this is a **follow-up task** requiring the `apiPost` layer from Task 12 plus a form, confirmation, and cache invalidation. Do not render a dispute control that does nothing.
