...
import { approveScopeVersion } from "@/lib/scope/service";
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ versionId: string }> }) {
  ...
  if (!can(ctx, "scope.approve")) return NextResponse.json({ error: "Forbidden" }, { status: 403 });
  const { versionId } = await params;
  try {
    const version = await approveScopeVersion(ctx, versionId);
