...
export default async function CustomerReportDetailPage({ params }: { params: Promise<{ reportId: string }> }) {
  ...
  const qsaStatus = (await listCustomerReportQsaStatuses(ctx, [report.id])).get(report.id)?.status ?? null;
